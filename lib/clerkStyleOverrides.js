...
export function applyUserProfileStyles(modalRootElement) {
    ...
    // Log the retrieved text color at the beginning
    console.log('[JS Override] applyUserProfileStyles called. Current --text-color from JS:', textColor);
    ...
    setTimeout(() => {
        ...
        // especially if identified by `div[data-clerk-modal="userProfile"] .cl-modal-content` or `.cl-userProfileSheet`.
        // The CSS targets these outer wrappers.

        console.log('[Clerk Style Override] Applying glassmorphism to modal wrapper:', modalRootElement);
    ...
    // Ensure its direct background color is transparent if glassBgColor has transparency
    modalRootElement.style.setProperty('background-color', 'transparent', 'important');
    ...
    textSelectors.forEach(selector => {
        modalRootElement.querySelectorAll(selector).forEach(el => {
            ...
            if (isTextTag || isSpecificClerkTextClass || (el.tagName === 'DIV' && el.children.length === 0 && el.textContent.trim().length > 0) ) {
                // Enhanced Logging for Text Color Application
                const originalComputedColor = getComputedStyle(el).color;
                console.log('[JS Override] Intending to apply color:', textColor, 'to element:', el, 'Current computed color:', originalComputedColor);
    ...
    console.log(`[Clerk Style Override] Finished applying text color "${textColor}" to text elements.`);
