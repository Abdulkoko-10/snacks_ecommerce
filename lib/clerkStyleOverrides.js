        ...
        if (document.documentElement.classList.contains('rgb-mode')) {
                ...
                if (elementsFound.length === 0) {
                    ...
                    elementsFound.forEach(el => {
                        ...
                        if (isTextTag || isSpecificClerkTextClass || (el.tagName === 'DIV' && el.children.length === 0 && el.textContent.trim().length > 0) ) {
                            ...
                            el.style.setProperty('outline', 'none', 'important');
                            el.style.setProperty('text-decoration', 'none', 'important');
                            // console.log('[JS Override RGB] Removed debug outline and text-decoration from:', el); // Log removed as per simplification
                        }
                    });
                }
            });
            console.log(`[Clerk Style Override] Finished applying RGB text color "${explicitTextColor}".`); // Simplified log
            ...
            textSelectors.forEach(selector => {
                modalRootElement.querySelectorAll(selector).forEach(el => {
                    ...
                    if (isTextTag || isSpecificClerkTextClass || (el.tagName === 'DIV' && el.children.length === 0 && el.textContent.trim().length > 0) ) {
                        // Simplified logging - keep essential info
                        // const originalComputedColor = getComputedStyle(el).color; // Removed for simplification
                        ...
                        // console.log('[JS Override] Computed color immediately after setProperty:', getComputedStyle(el).color); // Removed for simplification
                    }
                });
            });
...
export function observeUserProfileModal() {
    ...
    const callback = function(mutationsList, obs) {
        for(const mutation of mutationsList) {
            ...
            // Optional: Keep a simplified attribute mutation handler if desired,
            // but the primary trigger will be modal addition for this simplified approach.
            // For now, we can comment out or simplify the 'attributes' part to reduce complexity.
            /*
                ...
                if (modalRootElement && getComputedStyle(modalRootElement).display !== 'none') { // Check visibility
                    console.log('[JS Override] Attribute mutation on visible User Profile modal. Target:', mutation.target,'. Re-applying styles.');
                    applyUserProfileStyles(modalRootElement); // Re-apply on any style/class change
                }
            }
            */
        }
