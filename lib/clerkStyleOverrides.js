                ...
                if (elementsFound.length === 0) {
                    ...
                    elementsFound.forEach(el => {
                        ...
                        if (isTextTag || isSpecificClerkTextClass || (el.tagName === 'DIV' && el.children.length === 0 && el.textContent.trim().length > 0) ) {
                            ...
                            // NEW: Force other visual style changes for RGB mode text elements
                            console.log('[JS Override RGB] Attempting to apply debug outline and underline to:', el);
                            el.style.setProperty('outline', '2px solid lime', 'important');
                            el.style.setProperty('text-decoration', 'underline lime', 'important');
                            // Check if these debug styles were applied
                            console.log('[JS Override RGB] Applied outline:', el.style.outline, 'Applied text-decoration:', el.style.textDecoration);
                            const computedOutline = getComputedStyle(el).outline;
                            const computedTextDecoration = getComputedStyle(el).textDecoration;
                            console.log('[JS Override RGB] Computed outline after setProperty:', computedOutline, 'Computed text-decoration:', computedTextDecoration);
                        }
                    });
                }
            });
